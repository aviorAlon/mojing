...
Windows 也可以用 `scripts\setup.ps1` / `scripts\start.ps1`，Linux / macOS 用 `scripts/setup.sh` / `scripts/start.sh`。安装有问题先运行 `python scripts/doctor.py`，再看 [故障排查](docs/troubleshooting.md)。

## 文档

| 文档 | 内容 |
|---|---|
| [部署指南](docs/deployment.md) | 硬件要求、安装选项、局域网访问、Linux 服务器 / 云 GPU 部署、升级与卸载 |
| [架构说明](docs/architecture.md) | 组件、请求流程、换装管线（遮挡策略、按尺码表定长、质检）、性能数据、关键技术选型 |
| [数据模型](docs/data-model.md) | 商品（电商字段）和人物模型、字段来源标注、如何接入真实电商数据 |
| [校验与评分](docs/validation-and-scoring.md) | 照片检查、身材数据可信度、尺码选择、可信度评分和结果质检规则 |
| [生成模型配置](docs/generation.md) | 接入自己的图片 / 视频模型，生成模拟商品、模拟模特、转身视频 |
| [API](docs/api.md) | 所有 HTTP 接口 |
| [开发指南](docs/development.md) | 目录结构、测试、模拟商品工具、如何扩展 |
...
详见 [校验与评分](docs/validation-and-scoring.md) 和 [架构说明](docs/architecture.md)。
